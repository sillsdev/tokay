...
this.isFirefox=/gecko/.test(userAgent);
...
var isLocalResource=!resourceDomain||isLocalURL(url)||resourceDomain==Firebug.context.window.location.host;
...
length+=title.length;
for(var name in object){if(title==="History"){continue
}var value;
try{value=object[name]
}catch(exc){alert(title);
continue
...
}else{if(isExternal){var src=Firebug.Lite.Proxy.load(url);
renderProcess(src)
